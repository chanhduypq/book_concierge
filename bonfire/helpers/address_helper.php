...
if ( ! function_exists('state_select'))
{
	 ...
	 */
	function state_select($selected_code='', $default_abbr='', $country_code='', $select_name='state_code', $class='')
	{
		$CI =& get_instance();
		
		if (empty($country_code))
			$country_code = $CI->session->userdata('country') ? $CI->session->userdata('country') : 'US';
	
		// First, grab the states from the config
		//$all_states = config_item('address.states');		
		$CI->load->model('localization/region_model');
		$states = $CI->region_model->find_all_by('country_iso', $country_code);
		...
		if (is_array($states) && count($states)) {
			$output .= '<option '.(empty($selected_code) ? ' selected="selected"' : '').' value="">&nbsp;</option>' . PHP_EOL;
			foreach ($states as $state)
			{
				$output .= "<option value='{$state->id}'";
				$output .= ($state->id == $selected_code) ? ' selected="selected"' : '';
				$output .= ">{$state->name}</option>\n";
			}
		} else {
			$output .= '<option '.(empty($selected_code) ? ' selected="selected"' : '').' value="">'. lang('bf_select_no_state').'</option>' . PHP_EOL;
		}
		$output .= "</select>\n";
