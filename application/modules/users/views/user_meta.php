...
?>
<div class="row <?php echo iif( form_error('country') , 'error'); ?>" style="margin-bottom:15px;">
	<div class="col-sm-6">
		<label class="control-label" for="country"><strong><?php echo lang('user_meta_country'); ?></strong></label>
		<div class="controls">
			<?php echo country_select(set_value($field['name'], isset($user->$field['name']) ? $user->$field['name'] : $defaultCountry), $defaultCountry, 'country', 'span6 chzn-select form-control'); ?>
		</div>
	</div>
	<?php
			$field['name'] = 'state';
	...
	<div class="col-sm-6">
		...
		<div class="controls">
			<?php echo state_select(set_value($field['name'], $stateValue), $defaultState, $countryValue, $field['name'], 'span6 chzn-select form-control'); ?>
		</div>
	</div>
</div>
<?php
