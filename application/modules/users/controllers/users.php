    ...
    public function profile()
    {
        ...
        if (isset($_POST['save']))
        {
            ...
            if ($this->save_user($user_id, $meta_fields))
            {
				...
				$this->auth->update_extra_session($user_id);
        ...
        if ($settings['auth.password_show_labels'] == 1)
        {
	 	...
	 	if (!isset($user->hauth_provider) || empty($user->hauth_provider))
        ...
        try
        {
            ...
            if ($this->hybridauthlib->providerEnabled($provider))
            {
                ...
                if ($service->isUserConnected())
                {
					...
					if (empty($user_hauth->email))
						$user_hauth->email = $user_hauth->identifier.'@'.strtolower($provider).'.com';
