...
//end __construct()
    ...
    public function index($filter = 'all', $offset = 0)
    {
       ...
       if ($this->auth->role_id() > 1)
			$roles = $this->role_model->select('role_id, role_name')->where('role_id >', 1)->where('deleted', 0)->find_all();
		else
        ...
        if ($this->auth->role_id() > 1)
			Template::set('users', $this->user_model->where('users.role_id > ', 1)->find_all());
		else
