...
//end delete_autologin()
    ...
    private function setup_session($user_id, $username, $password_hash, $email, $role_id, $remember = FALSE, $old_token = NULL, $user_name = '')
    {
		...
		$this->update_extra_session($user_id);
	...
	public function update_extra_session($user_id)
	{
		$meta = $this->ci->user_model->find_user_and_meta($user_id);
		
		$user_country = $user_state = '';
		if (isset($meta->country) && !empty($meta->country)) {
			// load country
			$this->ci->load->config('address');
			$countries = config_item('address.countries');
			$user_country = (isset($countries[$meta->country]) ? $countries[$meta->country]['printable'] : 'United States');
			
			$this->ci->load->model('localization/region_model');
			$region = $this->ci->region_model->find($meta->state);
			if ($region)
				$user_state = $region->name;
				
			if ($user_state == $user_country) $user_state = '';
		}
    ...
    private function get_hauth_user()
    {
        ...
        if (count($providers) == 1)
        {
            try {
				...
				{
					$profile = $pro_storage->getUserProfile();
					
					$email = $profile->email;
					if (empty($email))
						$email = $profile->identifier.'@'.strtolower($providers[0]).'.com';
						
					$this->ci->benchmark->mark('hybridauth_profile_start');
					$user = $this->ci->user_model->find_by('email', $email);
					...
					if ($user && $user->hauth_provider != $providers[0])
					{
						$this->ci->load->library('Template');
