...
?>
<div class="admin-box">
	<h3>Currencies | <?php echo anchor(SITE_AREA.'/settings/localization/create_currency', 'add new'); ?></h3>
	<?php echo form_open($this->uri->uri_string()); ?>
		<table class="table table-striped">
			...
			<tbody>
				...
				<?php if ($can_edit) : ?>
					<td><?php echo anchor(SITE_AREA . '/settings/localization/edit_currency/' . $record->iso, $record->iso); ?></td>
				<?php else : ?>
					<td><?php e($record->iso); ?></td>
				<?php endif; ?>
					<td><?php e($record->default_country) ?></td>
					<td><?php if ($record->iso == 'USD') echo '<i class="glyphicon glyphicon-star"></i>' ?></td>
				<?php if ($can_edit) : ?>
					<td><?php echo anchor(SITE_AREA . '/settings/localization/edit_currency/' . $record->iso, '<span class="icon-pencil"></span>'); ?></td>
				<?php endif; ?>
				</tr>
				<?php
					endforeach;
