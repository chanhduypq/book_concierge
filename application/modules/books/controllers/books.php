...
class books extends Front_Controller
{
	...
	public function __construct()
	{
		...
		$this->load->helper('books');		
                ...
                $this->available_categories = array('default'=>0, 'children'=>4, 'management'=>3, 'religion'=>22, 'crime'=>18);
	...
	public function search($q = '', $offset=0, $category_id = 0)
	{
		if ($this->input->get('q'))
			$q = $this->input->get('q');
		...
		if (!$total_records) {
			...
			$ranker = 'sum((4*lcs+2*(min_hit_pos==1)+exact_hit)*user_weight)*1000+bm25';
                        ...
                        if((int)$category_id > 0){
                            $this->sphinxsearch->SetFilter('top_category',array((int)$category_id));
                            $category = array_search($category_id, $this->available_categories);
                        }
			...
			if (!empty($results) && isset($results['matches']) && count($results['matches'])) { 
				...
				$pager = array();
				$pager['base_url'] = site_url((empty($category) || $category == 'default' ? 'books/search' : 'category/'.$category.'/search')."/$q/");
				$pager['total_rows'] = $total_records;
				$pager['per_page'] = $itemsPerPage;
				$pager['uri_segment']	= (empty($category) || $category == 'default') ? 4 : 5;
        ...
        public function category_search($category, $q="", $offset=0) {
            if (!array_key_exists($category, $this->available_categories))
                $category = 'default';
            
            $category_id = $this->available_categories[$category];
            
            Template::set_view('search');
            
            $this->search($q, $offset, $category_id);
        }
